...
void JsonDataReader::ProcessStatRequests(const std::shared_ptr<MapRenderer> renderer) {
    const json::Node& stat_req_node = root_node_.AsDict().at("stat_requests").AsArray();
    //ParseStatRequests(stat_req_node, renderer);
    BuildStatsForJson(stat_req_node, renderer);
}
...
void JsonDataReader::BuildStatsForJson(const json::Node& node, const std::shared_ptr<MapRenderer> renderer){
    for (auto& request : node.AsArray()) {
...
json::Node JsonDataReader::FormMapResponce(const json::Node& node, const std::shared_ptr<MapRenderer> renderer) {
    json::Dict responce;
