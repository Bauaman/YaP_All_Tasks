...
public:
    ...
    //Разрешить компилятору сгенерировать перемещающий конструктор
    Witch(Witch&&) = default;

    //Разрешить компилятору сгенерировать оператор перемещающего присваивания
    Witch& operator=(Witch&&) = default;

    //Конструктор копирования
    explicit Witch(Witch& other) {
        name_ = other.name_;
        Cat new_cat(other.GetCat()->GetName(), other.GetCat()->GetAge());
        cat_ = make_unique<Cat>(new_cat);
    }

    Witch& operator=(Witch& rhs) {
        Witch temp(rhs);
        swap(name_, temp.name_);
        swap(cat_, temp.cat_);
        return *this;
    }
