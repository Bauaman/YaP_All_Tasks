...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
    #ifdef _MSC_VER
        if ((outfile = _wfopenfile.string().c_str(), "wb")) == NULL) {
    #else
    ...
    */
     ...
     */
        std::vector<JSAMPLE> image_buffer;
        const Color* line = image.GetLine(line_index++);
        
        for (int pos = 0; pos < image.GetWidth(); ++pos) {
            image_buffer.push_back(static_cast<JSAMPLE>(line[pos].r));
            image_buffer.push_back(static_cast<JSAMPLE>(line[pos].g));
            image_buffer.push_back(static_cast<JSAMPLE>(line[pos].b));
        }
        row_pointer[0] = row_pointer[0] = image_buffer.data();
     ...
     * more than one scanline at a time if that's more convenient.
     *//*
        const Color* line_begin = image.GetLine(cinfo.next_scanline);
        const Color* line_end = line_begin + image.GetWidth();
        int i=0;
        std::vector<JSAMPLE> image_buffer(row_stride);
        for (;line_begin < line_end; ++line_begin) {
            image_buffer[i] = static_cast<JSAMPLE>(line_begin->r);
            image_buffer[i+1] = static_cast<JSAMPLE>(line_begin->g);
            image_buffer[i+2] = static_cast<JSAMPLE>(line_begin->b);
            i+=3;
        }
