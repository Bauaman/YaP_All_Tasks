...
void JsonDataReader::ProcessStatRequests(const std::shared_ptr<MapRenderer> renderer, const std::shared_ptr<TransportRouter> router) {
    const json::Node& stat_req_node = root_node_.AsDict().at("stat_requests").AsArray();
    //ParseStatRequests(stat_req_node, renderer);
    BuildStatsForJson(stat_req_node, renderer, router);
}
...
void JsonDataReader::BuildStatsForJson(const json::Node& node, const std::shared_ptr<MapRenderer> renderer, const std::shared_ptr<TransportRouter> router){
    for (auto& request : node.AsArray()) {
        const auto& req_ = request.AsDict();
        if (req_.at("type").AsString() == "Bus") {
            responces_.emplace_back(FormBusResponceBuilder(req_));
        } else if (req_.at("type").AsString() == "Stop") {
            responces_.emplace_back(FormStopResponceBuilder(req_));
        } else if (req_.at("type").AsString() == "Map") {
            responces_.emplace_back(FormMapResponceBuilder(req_, renderer));
        } else if (req_.at("type").AsString() == "Route") {
            responces_.emplace_back(FormRouteResponceBuilder(req_, router));
        }
    }
}

json::Node JsonDataReader::FormRouteResponceBuilder(const json::Node& node, const std::shared_ptr<TransportRouter> router) {
    json::Dict responce;
    const std::string_view stop_from = node.AsDict().at("from").AsString();
    const std::string_view stop_to = node.AsDict().at("to").AsString();
    const auto& routing = router->FindRoute(stop_from, stop_to);
    if (!routing) {
        return FormErrorResponceBuilder(node.AsDict().at("id").AsInt());
    } else {
        json::Array items;
        double total_time =0.;
        ...
        for (auto& edge_id : routing.value().edges) {
            ...
            items.emplace_back(json::Node(json::Builder{}
                                .StartDict()
                                    .Key("stop_name").Value(router.get()->GetVertexById(edge.from))
                                    .Key("time").Value(router.get()->GetWaitTime())
                                    .Key("type").Value("Wait")
                                .EndDict()
                            .Build().AsDict()
                            )
                        );
            //total_time += router.get()->GetWaitTime();
            items.emplace_back(json::Node(json::Builder{}
                                .StartDict()
                                    .Key("bus").Value(edge.name)
                                    .Key("span_count").Value(static_cast<int>(edge.qual))
                                    .Key("time").Value(edge.weight-router.get()->GetWaitTime())
                                    .Key("type").Value("Bus")
                                .EndDict()
                            .Build().AsDict()
                            )                           
        ...
        responce = json::Builder{}
                        .StartDict()
                            .Key("request_id"s).Value(node.AsDict().at("id").AsInt())
...
json::Node JsonDataReader::FormMapResponceBuilder(const json::Node& node, const std::shared_ptr<MapRenderer> renderer) { 
    json::Dict responce;
    ...
    responce = json::Builder{}
                    .StartDict()
                        .Key("request_id").Value(node.AsDict().at("id").AsInt())
                        .Key("map").Value(doc_sting)
                    .EndDict()
                .Build().AsDict();
    return responce;
}

json::Node JsonDataReader::FormBusResponceBuilder(const json::Node& node) {
    json::Dict responce;
    BusInfo bus_info;
    ...
    if (!bus) {
        return FormErrorResponceBuilder(bus_info.request_id_);
    }
    ...
    responce = json::Builder{}.
                        StartDict()
                        .Key("request_id").Value(bus_info.request_id_)
                        .Key("curvature").Value(bus_info.curvature)
                        .Key("route_length").Value(bus_info.route_length_)
                        .Key("stop_count").Value(bus_info.stops_count_)
                        .Key("unique_stop_count").Value(bus_info.unique_stops_count_)
                        .EndDict()
                    .Build().AsDict();
    return responce;
}

json::Node JsonDataReader::FormStopResponceBuilder(const json::Node& node) {
    json::Array buses;
    json::Dict responce;
...
json::Node JsonDataReader::FormErrorResponceBuilder(int request_id) {
    json::Dict responce;
    responce = json::Builder{}
                        .StartDict()
...
RoutingSettings JsonDataReader::ProcessRoutingSettings() {
    json::Node r_settings_node = root_node_.AsDict().at("routing_settings").AsDict();
    return RoutingSettings{
                .bus_velocity = r_settings_node.AsDict().at("bus_velocity").AsInt(),
                .bus_wait_time = r_settings_node.AsDict().at("bus_wait_time").AsInt()
    };
}