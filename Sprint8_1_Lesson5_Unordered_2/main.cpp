...
public:
    ...
    void Withdraw(const VehiclePlate& car) {
        ...
        if (now_parked_.count(car)) {
            Duration total_duration = chrono::duration_cast<chrono::seconds>(Clock::now() - now_parked_.at(car));
            complete_parks_[car] += total_duration;
    ...
    unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> EndPeriodAndGetBills() {
        // место для вашей реализации
        unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> end_period_bills;
        for (auto& [car, dur] : now_parked_) {
            if (now_parked_.count(car)) {
                complete_parks_[car] += chrono::duration_cast<chrono::seconds>(Clock::now() - dur);
                now_parked_[car] = Clock::now();
            }
        }
        for (const auto& [plate, dur] : complete_parks_) {
            int bill = GetCurrentBill(plate);
            end_period_bills.emplace(plate, bill);
        }
        complete_parks_.clear();
        return end_period_bills;
    }
